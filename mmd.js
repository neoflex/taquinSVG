...
function MMD() {
		...
		{	tabrecherche = tabrecherche.sort(triermel3);
			cycles++;
			...
			if(etatcourant.compare(etatfinal))
				{solutiontrouvee(etatcourant);ctrouve=true;break;}
						...
						if((etatcourant.trouc==1)&(etatcourant.troul!=1)&(etatcourant.troul!=nbcasel))
						{
							...
							etatd.echange('d');
							if(pasdejapresentDFS(etatd)){etatd.liste_mouv.ajout(etatd.troul,etatd.trouc);etatd.nbenplace();etatd.fixmel();enfilerDFS(etatd);}
							etath.egal(etatcourant);
							etath.echange('h');
							if(pasdejapresentDFS(etath)){etath.liste_mouv.ajout(etath.troul,etath.trouc);etath.nbenplace();etath.fixmel();enfilerDFS(etath);}
							etatb.egal(etatcourant);
							etatb.echange('b');
							if(pasdejapresentDFS(etatb)){etatb.liste_mouv.ajout(etatb.troul,etatb.trouc);etatb.nbenplace();etatb.fixmel();enfilerDFS(etatb);}
						}
						if((etatcourant.trouc==nbcasec)&(etatcourant.troul!=1)&(etatcourant.troul!=nbcasel))
						{
							etatg.egal(etatcourant);
							etatg.echange('g');
							if(pasdejapresentDFS(etatg)){etatg.liste_mouv.ajout(etatg.troul,etatg.trouc);etatg.nbenplace();etatg.fixmel();enfilerDFS(etatg);}
							etath.egal(etatcourant);
							etath.echange('h');
							if(pasdejapresentDFS(etath)){etath.liste_mouv.ajout(etath.troul,etath.trouc);etath.nbenplace();etath.fixmel();enfilerDFS(etath);}
							etatb.egal(etatcourant);
						...
						if((etatcourant.troul==1)&(etatcourant.trouc!=1)&(etatcourant.trouc!=nbcasec))
						{
							...
							etatd.egal(etatcourant);
							etatd.echange('d');
							if(pasdejapresentDFS(etatd)){etatd.liste_mouv.ajout(etatd.troul,etatd.trouc);etatd.nbenplace();enfilerDFS(etatd);}
							etatb.egal(etatcourant);
							etatb.echange('b');
							if(pasdejapresentDFS(etatb)){etatb.liste_mouv.ajout(etatb.troul,etatb.trouc);etatb.nbenplace();etatb.fixmel();enfilerDFS(etatb);}
						}
